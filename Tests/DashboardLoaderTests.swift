...
final class DashboardLoaderTests: XCTestCase {
    ...
    func testTopContactsLast30Days() throws {
        ...
        // Their counts in the 30d window:
        //   sent     = 3  (rows 100/101/102 — all coalesce to handle 1)
        //   received = 1  (row 103 only; row 104 has handle_id=2 → separate
        //                  bucket)
        //   total    = 4
        XCTAssertEqual(top.sent, 3, "Top contact sent count in 30d window.")
        XCTAssertEqual(top.received, 1, "Top contact received count in 30d window (handle 2 row is separate).")
        XCTAssertEqual(top.total, 4, "Top contact total in 30d window.")
        ...
        // Email handle 2 also appears as a separate entry with 1 received.
        XCTAssertTrue(names.contains("friend@example.com"),
                      "Unmerged email handle should appear separately.")
    }
